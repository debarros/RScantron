...
UpdateTab = function(missingTests, TestFrame, TAB.wb, TABpath, messageLevel = 0){
  ...
  if(length(missingTests) > 0){                                                  # If there are any missing tests,
    ...
    TestFrame.temp$Local.folder = NA_character_                                  # Add a Local.Folder column
    TestFrame.temp = TestFrame.temp[,c(3,1,2,4)]                                 # Rearrange the columns
    colnames(TestFrame.temp) = colnames(TAB)                                     # Make the column names match
    NewTestFrame = rbind.data.frame(TAB, TestFrame.temp)                         # Merge the two sets of tests
