...
FindResults_1test = function(tid, attempt = 1, messageLevel = 0, agent = NULL){
  
  if(is.null(agent)){
    agent = "Mozilla/5.0 (Windows NT 6.3; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/37.0.2049.0 Safari/537.36"
  }
  
  url = paste0('https://admin.achievementseries.com/published-test/export/csv-list.csv?',
               'id=', tid, '&d=F&t=T&r=F&p=F&_list=ExportList')
  scores = httr::content(
...
FindResults = function (testIDs, messageLevel = 0, agent = NULL){
  ...
  for (i in 1:length(testIDs)){
    ...
    if(nrow(x) > 0){
      ResultFrame = rbind(ResultFrame,x)
    }
  } # /for
