...
FindFolders = function (ScantronHandle,
                        type,
  ...
  for (i in 1:length(folderlinks)){              # fill up the location data.frame with starting points of the folder ID codes
    location[i,1] = as.integer(
      gregexpr(pattern = folderlinks[i],x)[[1]][1])
  }
  ...
  if(nrow(location)==0){                   # If there are no folders here,
    return(TempFolders)                             # return the empty data.frame
  }                              # If there are folders, continue to find them
  ...
  if(nrow(bounds)==0){    # If there are no folders remaining in the list,
    return(TempFolders)   # return the empty data.frame
  }
  ...
  for (i in 1:nrow(bounds)){ #for each folder found here
    ...
    FolderGrab = FindFolders(ScantronHandle, 
                                 type,
                                 SkipFolder, 
                                 x, 
                                 TempParent, 
                                 NextFolderID) #recursive call
